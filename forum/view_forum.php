...
if (isset($_GET['id'])) {
	...
	$replies = $conn->query("SELECT r.*,u.lastname,u.firstname FROM replies r inner join tenants u on u.house_id = r.user_id where r.comment_id in (SELECT id FROM comments where topic_id= " . $_GET['id'] . ") order by unix_timestamp(r.date_created) asc");
	$rep_arr = array();
	while ($row = $replies->fetch_assoc()) {
		$rep_arr[$row['comment_id']][] = $row;
	}
	if ($user_id != $_SESSION['login_UId'] ) {
		$chk = $conn->query("SELECT * FROM forum_views where  topic_id=$id and user_id='{$_SESSION['login_UId']}' ")->num_rows;
		if ($chk <= 0) {
			$conn->query("INSERT INTO forum_views set  topic_id=$id , user_id='{$_SESSION['login_UId']}' ");
		}
	}
				...
				<div class="col-lg-12">
					...
					<!--Comment section-->
						<div class="form-group comment">
							...
							<p class="mb-0">
								<large><b><?php echo $row['firstname'] ?></b></large> <span class="text-primary"><small class="mb-0"><i><?php echo $row['lastname'] ?></i></small></span>
							</p>
									...
									<div class="col-lg-8 offset-lg-2 replies">
										...
										?>
											<div class="form-group ty-compact-list">
												<?php if ($_SESSION['login_UId'] == $rep['user_id']) : ?>
													<div class="dropleft float-right">
														<a class="text-dark" href="javascript:void(0)" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
															<span class="fa fa-ellipsis-v"></span>
														</a>
												...
												<p class="mb-0">
													<large><b><?php echo $rep['firstname'] ?></b></large> <span class="text-primary"><small class="mb-0"><i><?php echo $rep['lastname'] ?></i></small></span>
												</p>
...
<script>
	...
	$('.edit_comment').click(function() {
		uni_modal("Edit Comment", "forum/manage_comment.php?id=" + $(this).attr('data-id'), 'mid-large')

	})
	$('.edit_reply').click(function() {
		uni_modal("Edit Reply", "forum/manage_reply.php?id=" + $(this).attr('data-id'), 'mid-large')
