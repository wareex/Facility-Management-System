	...
	<form action="" id="manage-tenant">
		...
		<div class="form-group row">
			...
			<div class="col-md-4">
				...
				<input type="text" class="form-control" name="job" value="<?php echo isset($job) ? $job : '' ?>" required>
			</div>
		</div>
		<div class="form-group row">
			<div class="form-group">
				<label class="control-label">Number of Residence Occupant</label>
				<input type="number" class="form-control text-right" name="no_of_occupant" step="any" required="">
			</div>
			<div class="col-md-4">
				<label for="" class="control-label">Property Occupied</label>
				<select name="house_id" id="" class="custom-select select2">
					<option value=""></option>
					<?php
					$house = $conn->query("SELECT * FROM houses where house_no not in (SELECT house_id from tenants where status = 1) " . (isset($house_id) ? " or id = $house_id" : "") . " ");
					while ($row = $house->fetch_assoc()) :
					?>
						<option value="<?php echo $row['house_no'] ?>" <?php echo isset($house_id) && $house_id == $row['house_no'] ? 'selected' : '' ?>><?php echo $row['house_no'] ?></option>
					<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-4">
	...
	$('#manage-tenant').submit(function(e) {
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Tenant Saved Successfully.", 'success')
					setTimeout(function() {
						location.reload()
					}, 1000)
				}
